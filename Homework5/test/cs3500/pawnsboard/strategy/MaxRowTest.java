...
public class MaxRowTest {
   ...
   * Uses the MockPawnsBoardModel and enables transcript recording.
   */
  @Before
  public void setUp() {
  ...
  public void testChooseMoveReturnsNullWhenNoLegalMoves() {
    ...
    // Verify that getLegalMoves was called.
  ...
  public void testChooseMoveWinsRow() {
    ...
    // Place a Blue card in row 0 to make that row unwinnable.
    ...
    // Check that key model methods were called during simulation.
    List<String> transcript = model.getTranscript();
    assertTrue("Transcript should record getLegalMoves", transcript.contains("getLegalMoves called"));
    assertTrue("Transcript should record computeRowScores", transcript.contains("computeRowScores called"));
    assertTrue("Transcript should record getBoard", transcript.contains("getBoard called"));
    assertTrue("Transcript should record at least one cloneBoard call",
            transcript.stream().filter(s -> s.equals("cloneBoard called")).count() >= 1);
    assertTrue("Transcript should record getCurrentPlayer", transcript.stream().anyMatch(s -> s.startsWith("getCurrentPlayer called")));
  }
  ...
  public void testReturnsNullWhenNoRowFlipPossible() {
    ...
    // Verify that at least computeRowScores and getLegalMoves were invoked.
    List<String> transcript = model.getTranscript();
    assertTrue("Transcript should contain computeRowScores call", transcript.contains("computeRowScores called"));
    assertTrue("Transcript should contain getLegalMoves call", transcript.contains("getLegalMoves called"));
  }
  ...
  public void testMultipleRowsSimulationOrder() {
    ...
    // Verify that the transcript records the expected calls.
    List<String> transcript = model.getTranscript();
    assertTrue("Transcript should record getLegalMoves", transcript.contains("getLegalMoves called"));
    assertTrue("Transcript should record computeRowScores", transcript.contains("computeRowScores called"));
  }
